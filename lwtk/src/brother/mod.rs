...
#[derive(Debug, Clone, Copy)]
pub struct NumCmpLine<'arg> {
    // number-compare line
    cmp_exp: &'arg String,    // expression to compare;
    cmp_cell: (usize, usize), // (row_index,col_index) starts from (1,1);
}
pub fn extsn<'p>(ifp: &'p str) -> Option<&'p str> {
    match Path::new(ifp).extension() {
        Some(_has_extension) => {
            match _has_extension.to_str() {
                // &OsStr.to_str(&self)->Option<&str>
                Some(_any_extension) => Some(_any_extension),
                None => None,
            }
        }
        None => None,
...
// judge Vec<bool>:
pub fn vecb_judge<'mch>(
    // judge Vec<bool>;
    v: &'mch Vec<bool>,
) -> bool {
    v.par_iter().all(|&b| b)
}
pub fn value2str(cell_value: &Data) -> String {
    match cell_value {
            ...
            match _any_extension {
                "xlsx" => {
                    ...
                    match open_workbook::<Xlsx<_>, &Path>(path) {
                        Ok(mut _reader) => match _reader.worksheet_range(sht.shtna.as_str()) {
                ...
                "xls" => match open_workbook::<Xls<_>, &Path>(path) {
                    Ok(mut _reader) => match _reader.worksheet_range(sht.shtna.as_str()) {
                        Ok(_range) => return Ok(_range),
                        _ => return Err(format!("Invalid sheet: {}", sht.shtna)),
                    },
