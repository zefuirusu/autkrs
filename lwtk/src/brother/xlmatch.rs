...
use crate::brother::{get_sht_data, value2str, vecb_judge};
use crate::brother::{ShtMeta, StrMchLine};

// single row,single condition line:
fn single_row_match<'mch>(_row: &'mch [Data], _line: &'mch StrMchLine) -> bool {
    let match_cell = _row
        .get(_line.match_cell.1 as usize - 1)
...
fn multi_rows_match<'mch>(_row: &'mch [Data], _lines: &'mch Vec<StrMchLine>) -> bool {
    vecb_judge(
        &_lines
            .iter()
            .map(|line| single_row_match(_row, line))
...
fn adv_filter<'f>(
    ...
    _rg.rows()
        .par_bridge()
        .filter_map(|row| match multi_rows_match(&row, _lines) {
            true => Some(
                row.into_iter()
                    .map(|cell| value2str(cell))
                    .collect::<Vec<_>>(),
            ),
            false => Option::None,
        })
...
pub fn test_multi_match() -> () {
    ...
    let _lines: Vec<StrMchLine> = vec![
        StrMchLine {
            regex_str: &s1,
            match_cell: (1usize, 5usize),
        },
        StrMchLine {
            regex_str: &s2,
            match_cell: (1usize, 10usize),
        },
    ];
    let _shtli: Vec<ShtMeta> = vec![ShtMeta {
        ifp: &pstr,
        shtna: &s3,
    }];
    let resu: Vec<Vec<String>> = multi_sht_match(&_lines, &_shtli);
    println!("{:?}", &resu.len());
    for row in &resu {
        println!("{:?}", &row);
    }
...
pub fn test2_multi_match() -> () {
    ...
    let _lines: Vec<StrMchLine> = vec![
        StrMchLine {
            regex_str: &s1,
            match_cell: (1usize, 3usize),
        },
        StrMchLine {
            regex_str: &s2,
            match_cell: (1usize, 6usize),
        },
    ];
    let _shtli: Vec<ShtMeta> = vec![ShtMeta {
        ifp: &pstr,
